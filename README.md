...
function calculateModel(generateNewError = false) {
    ...
    // Mathematical calibration values (1-10 scale constants)
    const b0 = -11.11; 
    const b1 = 2.78;   
    const b2 = 3.33;   
    const b3 = 2.78;   
    ...
    // Compute final regression score including the active error term
    let totalTP = b0 + (b1 * pc) + (b2 * tc) + (b3 * px) + (b4 * sd) + activeErrorValue;

    // Safeguard mathematical boundaries (0% to 100%)
    ...
    document.getElementById('formula-output').innerHTML = 
        `<strong>Live Equation (with Error Term):</strong><br>` +
